(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))n(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerPolicy&&(i.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?i.credentials="include":o.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(o){if(o.ep)return;o.ep=!0;const i=s(o);fetch(o.href,i)}})();function A(){const e=document.getElementById("burgerToggle"),t=document.getElementById("burgerMenu"),s=document.querySelector("header");e.addEventListener("click",function(){t.classList.toggle("burger-menu-show"),t.classList.contains("burger-menu-show")?(e.innerHTML=`
        <svg width="18" height="18" stroke="#000">
          <use href="../img/icons.svg#icon-close"></use>
        </svg>`,s.classList.add("overflow-hidden"),document.body.classList.add("burger-menu-show")):(e.innerHTML=`
        <svg width="28" height="28">
          <use href="../img/icons.svg#icon-menu"></use>
        </svg>`,s.classList.remove("overflow-hidden"),document.body.classList.remove("burger-menu-show"))})}document.addEventListener("DOMContentLoaded",function(){A()});const p=window.location.pathname,c=document.querySelectorAll(".burger-menu-nav-link");let r=!1;c.forEach(e=>{const t=e.getAttribute("href");p.endsWith(t.slice(2))&&(e.classList.add("burger-menu-nav-link-active"),r=!0)});r||c[0].classList.add("burger-menu-nav-link-active");const v=window.location.pathname,h=document.querySelectorAll(".header-nav-link");let d=!1;h.forEach(e=>{const t=e.getAttribute("href");v.endsWith(t.slice(2))&&(e.classList.add("nav-link-active"),d=!0)});d||h[0].classList.add("nav-link-active");const g="data:image/png;base64,",u="data:image/png;base64,",b="/cod-cats/assets/icons-91f42774.svg#icon-close";function m({author:e,book_image:t,title:s,_id:n}){return`<li class="book-item" data-book-id="${n}">
              <div class="book-cover">
                  <img src="${t}" alt="book cover" />
                  <div class="quick-view">Quick view</div>
              </div>
              <div class="book-info">
                  <p class="book-title">${s}</p>
                  <p class="book-author">${e}</p>
              </div>
          </li>`}function f({image:e,title:t,category:s,description:n,author:o,buyLinks:i}){return`<li class="shop-list-book-card">
  <img src="${e}" class="shop-list-book-image">
  <div class="shop-list-book-info">
      <div class="shop-list-card-header">
          <div class="shop-list-book-title-container">
              <p class="shop-list-book-title">${t}</p>
              <p class="shop-list-book-genre">${s}</p>
          </div>
          <button type='button' class="delete-icon">
              <svg class="svg-icon" width="18" height="18" viewBox="0 0 18 18" fill="none">
                  <path d="M6.75 2.25H11.25M2.25 4.5H15.75M14.25 4.5L13.724 12.3895C13.6451 13.5732 13.6057 14.165 13.35 14.6138C13.1249 15.0088 12.7854 15.3265 12.3762 15.5248C11.9115 15.75 11.3183 15.75 10.132 15.75H7.86799C6.68168 15.75 6.08852 15.75 5.62375 15.5248C5.21457 15.3265 4.87507 15.0088 4.64999 14.6138C4.39433 14.165 4.35488 13.5732 4.27596 12.3895L3.75 4.5M7.5 7.875V11.625M10.5 7.875V11.625" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
          </button>
      </div>
      <p class="shop-list-book-description">${n}</p>
      <div class="shop-list-product-footer">
          <p class="shop-list-book-author">${o}</p>
          <div class="shop-list-book-stores">
              <a href="${i[0].url}" target="_blank"><img src="${g}" class="shop-list-amazon"></a>
              <a href="${i[1].url}}" target="_blank"><img src="${u}" class="shop-list-apple"></a>
          </div>
      </div>
  </div>
</li>`}function H({list_name:e,books:t}){const s=t.map(m).join("");return`<section class="books-category">
              <h3 class="category-title">${e}</h3>
              <ul class="books-list">${s}</ul>
              <button type="button" class="button" data-category="${e}">See more</button>
          </section>`}function L(e){return'<h1 class="main-title">Best Sellers <span>Books</span></h1>'+e.map(H).join("")}function k(e){const t=e.trim().split(" "),s=t.splice(t.length-1).join("");return`<h1 class="main-title">
          ${t.join(" ")} <span>${s}</span>
        </h1>`}function B(e,t){const s=k(t),n=e.map(m).join("");return`${s}
          <section class="books-category single">
            <ul class="books-list">${n}</ul>
          </section>`}function j({author:e,book_image:t,buy_links:s,description:n,title:o},i,l){return`<div class="modal">
      <button type="button" class="modal-btn">
        <svg class="modal-btn-icon" width="18" height="18">
          <use href="${b}"></use>
        </svg>
      </button>
      <div class="modal-wrap">
        <img src="${t}" class="modal-image" />
        <div class="modal-book-wrap">
          <h2 class="modal-book-title">${o}</h2>
          <p class="modal-book-author">${e}</p>
          <p class="modal-book-descr">
            ${n}
          </p>
          <div class="modal-logos-wrap">
            <a href="${s[0].url}" target="blank"><img src="${g}" alt="logo amazon" class="modal-shop-list-amazon" /></a>
            <a href="${s[1].url}" target="blank"><img src="${u}" alt="logo apple" class="modal-shop-list-apple" /></a>
          </div>
        </div>
      </div>
      <div class="modal-btn-add">
        <button type="button" class="${l.btnClass}" data-id="${i}">${l.btnText}</button>
        <p class="modal-btn-text">
          Сongratulations! You have added the book to the shopping list. To
          delete, press the button “Remove from the shopping list”.
        </p>
      </div>
    </div>`}function x(e){return e.map(f).join("")}const a=document.getElementById("toggleSwitch");a.addEventListener("change",Q);localStorage.getItem("darkMode")==="true"&&(a.checked=!0,document.body.classList.add("dark"));function Q(){a.checked?(document.body.classList.add("dark"),localStorage.setItem("darkMode","true")):(document.body.classList.remove("dark"),localStorage.setItem("darkMode","false"))}export{L as a,B as b,x as c,j as t};
//# sourceMappingURL=switch-theme-fd4a30d9.js.map
